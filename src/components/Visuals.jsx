...
function InfrastructureGraph() {
  ...
  return <div className="infra-visual"><div className="visual-glow" /><div className="infra-caption"><span className="status-dot" /> FINANCIAL INFRASTRUCTURE CONTROL CENTER</div><div className="infra-modules">{modules.map((module, index) => <span key={module} style={{ '--i': index }}>{module}</span>)}</div>{nodes.map((node, index) => <motion.div className="flow-node" key={node} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.12 }}><span className="node-index">0{index + 1}</span><strong>{node}</strong><span className="node-status">{index === nodes.length - 1 ? 'READY' : 'PASS'}</span>{index < nodes.length - 1 && <ArrowDown className="node-arrow" size={16} />}</motion.div>)}</div>
}
